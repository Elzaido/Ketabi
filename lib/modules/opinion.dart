  ...
  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit,HomeStates>(
        ...
        builder: (context, state){ return Scaffold(
            appBar: AppBar(
              title: const Text('رأيك يهمنا', style: TextStyle(
                fontFamily: 'Cairo',
              ),),
            ...
            body: Padding(
                ...
                child: Column(
                  children: [
                    descFormField(hint: 'أخبرنا برأيك في تطبيق كتابك', textController: opinionController),
                    const SizedBox(
                      height: 20,
                    ),
                    button(
                        onPressed: (){
                          if(opinionController.text == ''){
                            defaultToast(massage: 'لا يمكن إرسال الرأي فارغاً', state: ToastStates.ERROR);
                          }else{
                            HomeCubit.get(context).createOpinion(opinion: opinionController.text);
                          }
                        },
